...
| Feature 2 | — | **Portal Parceiro completo + novo Portal Fornecedor (Lavandaria):** **(1) Portal Parceiro (privacidade + criação de reservas)** — confirmado que as páginas `/parceiro/tarefas` e `/parceiro/reservas` **já não têm coluna "Funcionário"** (usam cards, não tabelas; só mostram Data, Propriedade, Hóspedes e Estado); o botão "Nova Reserva" e o modal já existem (propriedade_id filtrado, checkin, checkout, hospedes); o endpoint `POST /api/parceiro/reservas` já existe e já gera a Tarefa de limpeza para o checkout com `hospedes` e `estado: 'por_atribuir'`; os GETs do parceiro já filtram estritamente por `parceiro_id: req.user.id` (confirmado via grep); **criado o proxy route `/api/parceiro/[...path]/route.ts`** que faltava (as páginas chamavam `/api/parceiro/*` mas não havia proxy); corrigido bug latente em `parceiroController.listarTarefas` — o `.select()` não incluía `hospedes` (nem `roupa_entregue`/`sacos_roupa_suja`), agora inclui. **(2) Portal Fornecedor (novo role 'fornecedor')** — Schema `Utilizador`: enum `role` alargado para `['admin','gestor','staff','parceiro','fornecedor']`; Schema `Tarefa`: novos campos `roupa_entregue` (Boolean, default false) e `sacos_roupa_suja` (Number, default 0, min 0); backend: novo `middleware/requireRole.js` export `isFornecedor`; novo `controllers/fornecedorController.js` com `listarTarefas` (tarefas dos próximos 7 dias, filtro estrito `empresa_id`, populate `propriedade_id` com `capacidade_hospedes`) e `marcarRoupaEntregue` (toggle `roupa_entregue` com validação multi-tenant); novo `routes/fornecedorRoutes.js` montado em `/api/fornecedor` no `server.js`; frontend: tipo `Role` em `auth.ts` e `middleware.ts` alargado com `'fornecedor'`; `middleware.ts` protege `/fornecedor/*` (RouteGuard + matcher); novos proxy routes `/api/parceiro/[...path]` e `/api/fornecedor/[...path]`; nova página `/fornecedor/tarefas/page.tsx` com tabela (Data, Propriedade, Qtd. Hóspedes, Roupa Suja, Estado, Ação) e botão toggle "Marcar Entregue"; nova página raiz `/fornecedor/page.tsx` (redirect para `/fornecedor/tarefas`); novo layout `/fornecedor/layout.tsx` com `RouteGuard role="fornecedor"`. **(3) Controlo de Roupa pelo Staff** — `staffController.concluirTarefa` agora aceita `sacos_roupa_suja` no body e guarda em `tarefa.sacos_roupa_suja` (com validação); `detalhe-tarefa-client.tsx` ganhou input "Sacos de Roupa Suja Recolhidos" no modal de conclusão (envia no PATCH `/concluir`); o fornecedor vê este valor na coluna "Roupa Suja" da sua tabela. `tsc` 0 erros ✓ · `next lint` limpo ✓ · `node --check` backend ✓. |
| Feature 3 | — | **Sync integral parceiros, auto-atribuição, fallback hóspedes e role lavandaria:** **(1) Headers de logout** — confirmado que os layouts `/parceiro/layout.tsx` e `/fornecedor/layout.tsx` já incluem `PortalHeader` com `UserMenu` (avatar + dropdown "Perfil" + "Terminar Sessão") implementado na sessão anterior; sem alteração necessária. **(2) Role lavandaria no frontend** — selects de "Tipo de utilizador" no modal de criação e edição de Staff (`/gestor/equipa/page.tsx`) agora incluem a opção `Fornecedor (Lavandaria — gestão de roupa)` com value `fornecedor`. **(3) Fallback estrito de hóspedes** — `parceiroController.criarReserva`: o cálculo de `numHospedes` agora cobre `null`, `0`, `undefined` e `NaN` (antes só cobria `null`) → faz fallback a `propriedade.capacidade_hospedes`; `smoobuController.criarTarefaPorReserva` já cobria `null` e `0` (confirmado, sem alteração); a sincronização manual (`sincronizarReservas`) delega para `criarTarefaPorReserva`, pelo que o fallback já acontece. **(4) Sincronização integral Smoobu + Parceiros** — `smoobuController.sincronizarReservas` agora, após sincronizar o Smoobu, percorre todas as `ReservaManual` futuras cuja tarefa gerada não existe (útil após "Eliminar Futuras") e recria a Tarefa de limpeza para o check-out com `origem_parceiro: true` e fallback de `hospedes`; contadores `parceirosCriadas`/`parceirosSkipped` adicionados à resposta JSON; mensagem do toast atualizada para incluir parceiros; botão renomeado para "Sincronizar Limpezas (Smoobu + Parceiros)" em `/gestor/configuracoes/page.tsx` e `/gestor/configuracoes/integracoes/page.tsx`. **(5) Destaque e auto-atribuição de parceiros** — Schema `Tarefa`: novo campo `origem_parceiro` (Boolean, default false, indexado); `parceiroController.criarReserva` agora define `origem_parceiro: true` e invoca `determinarUtilizadorAtribuido` (ou `determinarEquipaAtribuida` se `staff_necessario > 1`) + `calcularInicioTarefaUtilizador` para auto-atribuir a tarefa a um funcionário (igual ao webhook do Smoobu); se não encontrar staff, fica `por_atribuir` com `motivo_nao_atribuicao`; frontend `/gestor/tarefas/page.tsx`: novo toggle "🤝 Apenas Parceiros" na barra de filtros + Badge "🤝 Parceiro" na coluna Propriedade quando `origem_parceiro` for true; `TarefaAdmin` alargado com `origem_parceiro?: boolean`. `tsc` 0 erros ✓ · `next lint` limpo ✓ · `node --check` backend ✓. |
