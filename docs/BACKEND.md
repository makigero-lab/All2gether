...
O backend tem dois cron jobs diários, ambos iniciados no arranque (`server.js`, dentro de `if (require.main === module)` — não correm nos testes):
...
> **Timezone:** o `Agenda de Amanhã` usa a opção `timezone: 'Europe/Lisbon'` do node-cron, pelo que as 19:00 são estáveis mesmo que o servidor esteja em UTC (caso do Render) — acompanha automaticamente as mudanças legais de horário de Verão/Inverno de Portugal. O `Daily Briefing` usa o fuso do servidor (definir `TZ=Europe/Lisbon` no ambiente para alinhar).
