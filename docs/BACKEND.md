...
> **Nota de rebranding.** O projeto foi consolidado como **All2gether** — sistema de gestão de tarefas para **Alojamento Local e Airbnb**. A antiga integração Smoobu (que convertia reservas em tarefas automaticamente) foi removida (F0). O endpoint de **receção** de webhooks foi re-introduzido como recetor/logger (Hotfix HF3) — grava payloads em `WebhookLog` para auditoria; a conversão reserva→tarefa fica como follow-up. Os modelos `Tarefa` (limpeza/manutenção) e `Propriedade` (alojamento) são o núcleo do domínio.
...
>
> **HF3 — Recetor de webhooks Smoobu re-introduzido (apenas receção + log).** O endpoint `POST /api/smoobu/webhook` volta a existir, mas **apenas como recetor/logger**: valida a autenticação via `SMOOBU_API_KEY` (headers `X-Smoobu-Api-Key` / `Api-Key` / `Authorization: Bearer`), grava o payload bruto em `WebhookLog` (best-effort, nunca crasha) e devolve 200. A **conversão reserva→tarefa** (que existia antes de F0 via `criarTarefaPorReserva`) **não foi re-implementada** — é um follow-up documentado. A rota está isenta do rate limiter global (webhooks M2M podem burstar de um IP único). Ver `backend/routes/smoobuRoutes.js`.
