...
// FIX (testes determinísticos) — Mock do geocoding (Nominatim/OSM) ao nível do
// MÓDULO para evitar chamadas de rede externa nos testes de integração.
// IMPORTANTE: Tem de ser jest.mock() e NÃO jest.spyOn(). O gestorController
// importa a função via destructuring (`const { obterCoordenadas } = require(...)`),
// pelo que jest.spyOn(geocoding, 'obterCoordenadas') substitui a propriedade no
// objeto do módulo MAS NÃO afeta a referência local já capturada pelo controller.
// jest.mock() é hoisted pelo babel-jest para ANTES dos requires, pelo que o
// controller recebe a função mockada quando faz o require. Sem isto, os testes
// que criam propriedades penduravam no timeout de 5s do Jest (Nominatim >5s
// ou sem rede no CI). O mock devolve coordenadas fixas de Lisboa (38.7223,
// -9.1393) para qualquer morada — os testes não validam coordenadas reais.
jest.mock('../utils/geocoding', () => ({
  obterCoordenadas: jest.fn().mockResolvedValue({ lat: 38.7223, lng: -9.1393 }),
}));
